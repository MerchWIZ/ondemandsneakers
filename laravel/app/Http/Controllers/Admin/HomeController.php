...
class HomeController extends AdminController {
    ...
    public function index() {
        ...
        $orders = Orders::where('paymentStatus', 'success')->get();
        $totalSuccessSales = $orders->count();

        $ordersPending = Orders::where('paymentStatus', 'pending')->get();
        $totalPendingSales = $ordersPending->count();

        $ordersTotal = Orders::get();
        $totalSales = $ordersTotal->count();
        $recentOrders = Orders::orderBy('orders.id', 'desc')->take(10)->get();
